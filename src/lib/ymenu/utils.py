...
def show_message (message, title='YMenu'):
...
def show_warning (message, title='YMenu'):
...
def show_question (message, title='YMenu'):
	...
	def notify (self, message, title='', icon='', timeout=-1):
		...
		try:
			if self.bus and self.notifications:
				self.notifications.Notify('YMenu', 0, icon, title, message,
					[], {}, timeout)
				return True
			else:
